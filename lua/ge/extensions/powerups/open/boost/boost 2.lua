...
M.onInit = function(group_defs)
	Sets.loadSet(M.file_path .. "sets/boost 2.lua", M.set_name)
end

-- Called for each vehicle
M.onVehicleInit = function(game_vehicle_id)
	be:getObjectByID(game_vehicle_id):queueLuaCommand('PowerUpSounds.addSound("' .. M.activate_sound .. '", "AudioSoft3D", 3, 1, "' .. M.file_path .. 'sounds/carrevving.ogg")')
end
...
M.whileActive = function(data, origin_id)
	if data.effect_timer:stop() > 2000 then return 1 end
end
