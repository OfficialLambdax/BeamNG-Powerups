...
M.onActivate = function(vehicle)
	...
	local targets = MathUtil.getVehiclesInsideRadius(vehicle:getPosition(), 100, origin_id)
	local targets = Extender.cleanseTargetsWithTraits(targets, origin_id, Trait.Ignore)
	local targets = Extender.cleanseTargetsBehindStatics(vehicle:getPosition(), targets) 
	
	local _, target_id = Util.tablePickRandom(targets)
	if not target_id then
		return onActivate.Error('No target available')
	end
