...
local M = {
	...
	clear_name = "Get Jinxed", -- Inspired by StreamerVsChat
	...
	set_name = "jinxed_" .. Util.randomName(),
}
...
M.onInit = function(group_defs)
	Sets.loadSet(M.file_path .. "sets/jinxed.lua", M.set_name)
end
...
M.onActivate = function(vehicle)
	...
	set:exec()
	return {timer = hptimer(), len = set:maxTime()}
end
...
M.whileActive = function(data, origin_id)
	if data.timer:stop() < data.len then return nil end
