...
local M = {
	...
	respects_traits = {Trait.Consuming, Trait.StrongConsuming},
...
M.onInit = function(group_defs)
	M.activate_sound = Sound(M.file_path .. 'sounds/shockwave_1.ogg', 6)
end
	...
	for _, target in pairs(getAllVehicles()) do
		...
		if vehicle_id ~= target_id then
			if Util.dist3d(vehicle:getPosition(), target:getPosition()) < M.effect_radius then
				if Extender.hasAnyTrait(target_id, Trait.Consuming, Trait.StrongConsuming) then
...
M.onHit = function(data, origin_id, target_id)
	...
	local push = (pos2 - pos1):normalized() * (math.min(M.effect_radius, M.effect_radius - Util.dist3d(pos2, pos1)) * 1)
