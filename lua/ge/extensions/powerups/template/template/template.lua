...
local M = {
	...
	-- This must match the power ups library M._NAME or this powerup is rejected.
	...
	force_field_set = "force_field_1_" .. Util.randomName(),
	force_field_sound = "force_field_1_" .. Util.randomName(),
	force_field_hit_sound = "force_field_1_hit" .. Util.randomName(),
}
...
M.onInit = function(group_defs)
	Sets.loadSet(M.file_path .. "sets/forceFieldBounce.lua", M.force_field_set)
end

-- Called for each vehicle
M.onVehicleInit = function(game_vehicle_id)
	be:getObjectByID(game_vehicle_id):queueLuaCommand('PowerUpSounds.addSound("' .. M.force_field_sound .. '", "AudioSoft3D", 12, 1, "' .. M.file_path ..'sounds/force field.ogg")')
...
M.whileActive = function(data)
	...
	--[[
		...
		X Return values
			nil = This effect is NOT over, just continue calling this effect
			1 = This effect is over, do nothing but call onDeactivate
			2 = This effect is over, call onTargetHit and onDeactivate
			3 = This effect is NOT over, but call onTargetHit
